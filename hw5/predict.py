...
def reconstruct(model, device, loader, n_iter, reduced_method, reduced_dim):
    model.eval()
    with torch.no_grad():
        for idx, (image_aug, image) in enumerate(loader):
...
if __name__ == '__main__':
    ...
    reconstruct(model, device, test_loader, NUM_ITER, reduced_method=REDUCED_METHOD, reduced_dim=REDUCED_DIM)
    predicted = clustering(model, device, test_loader, NUM_ITER, reduced_method=REDUCED_METHOD, reduced_dim=REDUCED_DIM)
    if (predicted[0] == 0):
        write_output(predicted, PRED_PATH)
    else:
        write_output(np.abs(1-predicted), PRED_PATH)